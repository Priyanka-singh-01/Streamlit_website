...
constant_fk2d=st.number_input('enter the value of fk2d (required = 0.75)', key='123') 
multiplying_factor_3d=st.number_input('enter the value of multiplying factor 3d (required = 0.5)', key='12') 
Shear_velocity=st.number_input('enter the value of shear velocity (required = 2.6**3)', key='shear_vel') 
...
num=st.number_input('choose the filtering method from above')
...
if tch:
    ...
    if sch:
        ...
        if sch> 7:
            ...
            files=st.file_uploader('upload all of ze files here', accept_multiple_files=True)
